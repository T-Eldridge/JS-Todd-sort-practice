...
const stringsToNumbs = (arr) => {
  nums = arr.map((numbers) => parseInt(numbers))
}
...
const everyWeatherState = []

const weatherStates = (arr) => {
  // eslint-disable-next-line no-plusplus
  for (let i = 0; i < arr.length; i++) {
    arr.forEach(() => {
      if (!everyWeatherState.includes(arr[i].weather_state_name)) {
        everyWeatherState.push(arr[i].weather_state_name)
      }
    })
  }
}
